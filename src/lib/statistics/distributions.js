...
exports.fDistributionCDF = fDistributionCDF;
exports.fPValue = fPValue;
exports.fCritical = fCritical;
exports.qCritical = qCritical;
exports.chiSquaredCDF = chiSquaredCDF;
 ...
 */
function closestDf(df, table) {
    var available = Object.keys(table).map(Number).sort(function (a, b) { return a - b; });
    if (df >= available[available.length - 1])
        return available[available.length - 1];
    if (df <= available[0])
        return available[0];
    var closest = available[0];
    for (var _i = 0, available_1 = available; _i < available_1.length; _i++) {
        var d = available_1[_i];
        if (d <= df)
            closest = d;
        else
            break;
 ...
 */
function qCritical(alpha, k, df) {
    var _a;
    var table = alpha <= 0.01 ? TUKEY_TABLE_001 : TUKEY_TABLE_005;
    var closeDf = closestDf(df, table);
    var row = table[closeDf];
    if (!row)
        return 3.0; // fallback
    var clampedK = Math.min(Math.max(k, 2), 20);
    return (_a = row[clampedK]) !== null && _a !== void 0 ? _a : 3.0;
}
// ============================================================
...
function regularizedGammaQ(a, x) {
    ...
    var result = Math.exp(-x + a * Math.log(x) - logGamma(a)) * h;
...
function normalCDF(x) {
    ...
    var erfApprox = t * Math.exp(-absX * absX - 1.26551223 +
        t * (1.00002368 +
            t * (0.37409196 +
                t * (0.09678418 +
                    t * (-0.18628806 +
